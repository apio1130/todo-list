...
class GroupsRepositoryTest {
    ...
    @BeforeEach
    void setUp() {
        groupsRepository.save(Groups.builder().title("할일").delYn("N").build());
    ...
    @Test
    void update() {
        Groups groups = groupsRepository.findById(1L).orElse(new Groups());
        groups.update(Groups.builder()
                .title("오늘 할일")
                .delYn(groups.getDelYn())
                .build());
